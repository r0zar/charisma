// Dependency injection interfaces for external dependencies
export interface VaultLiquidityProvider {
    getAllVaultData(): Promise<Vault[]>;
    getRemoveLiquidityQuote(contractId: string, amount: number): Promise<{ success: boolean; quote?: { dx: number; dy: number }; error?: string }>;
}

// Global provider instance - will be set by consuming app
let liquidityProvider: VaultLiquidityProvider | null = null;

export function setLiquidityProvider(provider: VaultLiquidityProvider) {
    liquidityProvider = provider;
}

// Vault interface from dex-cache
...
export const calculateLpIntrinsicValueFromVault = async (
    ...
    try {
        ...
        if (!liquidityProvider) {
            throw new Error('VaultLiquidityProvider not set - call setLiquidityProvider() before using LP calculator');
        }
        
        const quoteResult = await liquidityProvider.getRemoveLiquidityQuote(vault.contractId, lpAmountMicroUnits);
...
export const calculateLpIntrinsicValue = async (
    ...
    try {
        if (!liquidityProvider) {
            console.warn('[LP Calculator] VaultLiquidityProvider not set - call setLiquidityProvider() before using LP calculator');
            return null;
        }
        
        // Find the vault by contract ID
        const allVaults = await liquidityProvider.getAllVaultData();
 ...
 * @param timeoutMs - Timeout for processing (default: 15 seconds)
...
export const calculateAllLpIntrinsicValues = async (
    ...
    try {
        if (!liquidityProvider) {
            console.warn('[LP Calculator] VaultLiquidityProvider not set - call setLiquidityProvider() before using calculateAllLpIntrinsicValues');
            return new Map();
        }

        // Import the processing queue (dynamic import to avoid circular dependencies)
        const { LpProcessingQueue } = await import('./lp-processing-queue.js');
        
        const processor = new LpProcessingQueue();
        await processor.initialize(basePrices, liquidityProvider);
        const results = await processor.processAllLpTokens(timeoutMs);
        
        // Convert LpIntrinsicResult to the expected format
        const convertedResults = new Map<string, { usdPrice: number; sbtcRatio: number; confidence: number; level: number }>();
        results.forEach((result, contractId) => {
            convertedResults.set(contractId, {
        ...
        console.log(`[LP Calculator] calculateAllLpIntrinsicValues completed: ${convertedResults.size} LP tokens calculated`);
        return convertedResults;
        
    } catch (error) {
        console.error('[LP Calculator] Error in calculateAllLpIntrinsicValues:', error);
        return new Map();
    }
};