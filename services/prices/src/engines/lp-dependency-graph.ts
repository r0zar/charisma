...
// Vault interface for dependency graph
interface Vault {
    contractId: string;
    symbol: string;
    tokenA?: {
...
let vaultDataProvider: { getAllVaultData(): Promise<Vault[]> } | null = null;

export function setVaultDataProvider(provider: { getAllVaultData(): Promise<Vault[]> }) {
    vaultDataProvider = provider;
}
...
export class LpDependencyGraph {
    ...
    async buildGraph(): Promise<void> {
        if (!vaultDataProvider) {
            throw new Error('VaultDataProvider not set - call setVaultDataProvider() before using LP dependency graph');
        }

        const allVaults = await vaultDataProvider.getAllVaultData();
