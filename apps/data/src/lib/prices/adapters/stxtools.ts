/**
 * STXTools adapter - Oracle adapter that uses STXTools API for token prices
 */

import { listPricesSTXTools } from '@repo/tokens';
import type { IOracleAdapter } from '../types';

export class STXToolsAdapter implements IOracleAdapter {
  ...
  async getBtcPriceUsd(): Promise<number> {
    ...
    throw new Error('[STXToolsAdapter] No BTC price data available from STXTools');
  }
  ...
  private async fetchPricesFromSTXTools(): Promise<void> {
    try {
      console.log('[STXToolsAdapter] Fetching price data from STXTools API...');
      ...
      console.log(`[STXToolsAdapter] Cached ${tokenCount} token prices from STXTools API`);
      
      if (tokenCount === 0) {
        console.warn('[STXToolsAdapter] No prices returned from STXTools API');
      }
      
    } catch (error) {
      console.error(`[STXToolsAdapter] Failed to fetch price data:`, error);
      throw error;
    }
  }
