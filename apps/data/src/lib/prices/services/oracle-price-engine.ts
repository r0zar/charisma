...
import type { IOracleAdapter, OraclePriceConfig, OraclePriceResult } from '../types';
...
export class OraclePriceEngine implements IOraclePriceEngine {
  ...
  async getPrice(tokenId: string): Promise<OraclePriceResult | null> {
    ...
    const blobPath = `prices/current/${tokenId}.json`;
    const now = Date.now();

    // Check blob storage first
    if (this.blobStorage) {
      try {
        const storedPrice = await this.blobStorage.get<CachedPrice>(blobPath);
        if (storedPrice && now - new Date(storedPrice.timestamp).getTime() < this.config.timeoutMs) {
  ...
  async saveSnapshot(prices: Map<string, OraclePriceResult>): Promise<void> {
    try {
      const snapshot: Record<string, TokenPriceEntry> = {};
      
      for (const [tokenId, result] of prices) {
        snapshot[tokenId] = {
          tokenId,
          usdPrice: result.price.usdPrice,
          sbtcRatio: result.price.sbtcRatio,
          source: result.oracleResults.filter(r => r.success).map(r => r.adapterName).join(','),
          timestamp: result.metadata.timestamp
        };
      }
      
      await this.blobStorage.put('prices/snapshot.json', snapshot);
      console.log(`[OraclePriceEngine] Saved price snapshot for ${prices.size} tokens`);
    } catch (error) {
      console.error('[OraclePriceEngine] Error saving price snapshot:', error);
    }
  }
