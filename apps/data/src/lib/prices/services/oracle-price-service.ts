...
import { 
  ...
  defaultOracleConfig
} from './oracle-price-engine';
import { CoinGeckoAdapter, KrakenAdapter, STXToolsAdapter, InvestAdapter } from '../adapters';
import { blobStorageService } from '../../storage/blob-storage-service';
import type { PriceServiceResult } from '../types';
...
class OraclePriceService {
  ...
  private readonly PRICE_CACHE_KEY = 'prices/current';
  ...
  private registerAdapters(): void {
    try {
      ...
      // STXTools adapter
      const stxToolsAdapter = new STXToolsAdapter();
      this.engine.registerAdapter(stxToolsAdapter);

      // INVEST adapter
      const investAdapter = new InvestAdapter();
      this.engine.registerAdapter(investAdapter);
