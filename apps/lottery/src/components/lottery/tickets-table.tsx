...
import { Loader2, ExternalLink, CheckCircle2, ChevronDown, ChevronRight, Copy, X } from 'lucide-react'
  ...
  return (
    <>
      <div className="grid grid-cols-12 gap-3 px-3 py-2 hover:bg-muted/50 rounded-lg border-b border-border/20 items-center">
        ...
        <div className="col-span-4">
          <div className="text-xs text-muted-foreground">
            {tickets.length} tickets with random numbers
          </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-3 px-3 pb-2">
          <div className="col-span-12">
            <div className="bg-muted/30 rounded-lg p-3">
              <div className="text-xs font-medium text-muted-foreground mb-2">Individual Ticket Numbers:</div>
...
export function TicketsTable({ tickets, onConfirmationUpdate, onBulkConfirmationUpdate, onTicketCancelled, onBulkTicketsCancelled }: TicketsTableProps) {
  if (tickets.length === 0) {
    return (
