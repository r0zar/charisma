...
interface SubnetTokenSelectionStepProps {
...
const SubnetTokenSelectionStep = ({
    onSelectToken,
    isLoadingTokens,
    predefinedTokens,
    onCustomSubmit,
    sourceTokenSymbol
}: SubnetTokenSelectionStepProps) => {
    ...
    useEffect(() => {
        const subnetTokens = predefinedTokens.filter(token => token.type === 'SUBNET');
        setFilteredTokens(subnetTokens);
    }, [predefinedTokens]);
    ...
    useEffect(() => {
        ...
        const filtered = predefinedTokens
            .filter(token => token.type === 'SUBNET')
            .filter(token =>
    ...
    const fetchTokenMetadata = async (contractId: string) => {
        ...
        if (!/^[A-Z0-9]+\.[a-zA-Z0-9-]+$/.test(contractId)) {
            setCustomError("Invalid contract address format. Should be like: SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.subnet-token");
            return;
        }
        if (!contractId.toLowerCase().includes('subnet')) {
            setCustomError("Contract must be a subnet token (containing 'subnet' in its name)");
            return;
        }
        setCustomError(null);
        setIsFetchingToken(true);
        try {
            const result = await getTokenMetadataCached(contractId);
    ...
    const handleCustomSubmit = () => {
        if (!fetchedToken) {
            setCustomError("Please fetch a valid subnet token first");
            return;
        }
        setCustomError(null);
    ...
    return (
        <div className="space-y-8">
            <div className="text-center">
                ...
                <p className="text-muted-foreground">Choose the subnet token to link with {sourceTokenSymbol}</p>
            </div>
            {!showCustomInput && (
                ...
                <Card className="p-6">
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            <Label htmlFor="customAddress">Subnet Contract ID</Label>
                            <Input
                                id="customAddress"
                                value={customAddress}
                                onChange={(e) => {
                                ...
                                placeholder="e.g. SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.token-subnet"
                            />
                        </div>
                        <div>
                            <Button
                            ...
                            >
                                {isFetchingToken ? (
                                    <>
                                        ...
                                        Fetching Subnet Token...
                                    </>
                                ) : (
                                    <>Fetch Subnet Token Details</>
                                )}
                            </Button>
                        </div>
                        {fetchedToken && (
                ...
                <div>
                    {isLoadingTokens ? (
                        ...
                        <div className="text-center py-8 space-y-4">
                            <p className="text-muted-foreground">No subnet tokens found.</p>
                            <Button
                                variant="outline"
                                onClick={() => setShowCustomInput(true)}
                                className="mx-auto"
                            >
                                Enter Custom Subnet Token
                            </Button>
                        </div>
                    ) : (
...
export default SubnetTokenSelectionStep; 