...
;; Vault Metadata
(define-data-var token-uri (optional (string-utf8 256)) (some u"{{{METADATA_URI}}}"))
...
(define-public (deposit (amount uint) (recipient principal))
    (begin
        (try! (contract-call? '{{SUBNET_CONTRACT}} deposit amount (some recipient)))
