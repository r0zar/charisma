...
export class ApiClient {
   ...
   */
  async fetchMetadata(useDefault = false): Promise<StateResult<AppState['metadata']>> {
    try {
   ...
   */
  async fetchUser(useDefault = false, section?: 'settings' | 'wallet' | 'preferences', userId?: string): Promise<StateResult<AppState['user']>> {
    try {
  ...
  async fetchBots(params: {
    ...
    limit?: number;
    offset?: number;
  } = {}): Promise<StateResult<AppState['bots']>> {
    try {
  ...
  async fetchMarket(params: {
    useDefault?: boolean;
    section?: 'data' | 'analytics' | 'pools';
    limit?: number;
    sortBy?: 'apr' | 'tvl' | 'volume';
    sortOrder?: 'asc' | 'desc';
  } = {}): Promise<StateResult<AppState['market']>> {
    try {
      const url = new URL(`${this.config.baseUrl}/market`);
  ...
  async fetchNotifications(params: {
    ...
    offset?: number;
    userId?: string;
  } = {}): Promise<StateResult<AppState['notifications']>> {
    try {
