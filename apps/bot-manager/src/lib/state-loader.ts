...
// Validate state without loading (for testing)
export async function validateStateFile(path: string): Promise<StateValidationResult> {
  try {
    let data: any;
    ...
    if (path.startsWith('http://') || path.startsWith('https://')) {
      ...
      if (!response.ok) {
        return {
          isValid: false,
          errors: [`Failed to load state from ${path}: ${response.status} ${response.statusText}`],
          warnings: [],
          metadata: {
            version: 'unknown',
            botCount: 0,
      ...
      try {
        ...
        return {
          isValid: false,
          errors: [`Failed to read file ${path}: ${fileError instanceof Error ? fileError.message : String(fileError)}`],
          warnings: [],
          metadata: {
            version: 'unknown',
            botCount: 0,
    ...
    return {
      isValid: false,
      errors: [`Failed to parse state from ${path}: ${error instanceof Error ? error.message : String(error)}`],
      warnings: [],
      metadata: {
        version: 'unknown',
        botCount: 0,
