...
export interface EncryptedWalletData {
  ...
  publicKey: string; // Public key is not encrypted (safe to store)
}
...
export function encryptWalletCredentials(credentials: WalletCredentials): EncryptedWalletData {
  ...
  return {
    ...
    publicKey: credentials.publicKey // Public key stored unencrypted (safe)
  };
}
...
export async function generateBotWallet(): Promise<WalletCredentials> {
  ...
  const { getAddressFromPrivateKey, privateKeyToPublic } = await import('@stacks/transactions');
  ...
  // Use mainnet for SP addresses instead of testnet ST addresses
  const walletAddress = getAddressFromPrivateKey(privateKey, STACKS_MAINNET);
