...
async function main() {
  try {
    ...
    await logExecution('Starting transaction events lookup', 'Fetch real blockchain transaction events for wallet address');
    ...
    await logger.info('Fetching transaction events', {
      address,
      params: queryParams,
    ...
    await logResult('Transaction events lookup', {
      exitCode: 0,
      stdout: 'Transaction events fetched successfully',
      summary: {
        address,
        eventCount: response.events?.length || 0,
        responseSize: `${(JSON.stringify(response).length / 1024).toFixed(2)} KB`,
        duration: `${duration}ms`,
        apiLimit: response.limit,
        apiOffset: response.offset
      }
    }, duration);
    ...
    await logError('Transaction events lookup failed', error instanceof Error ? error : new Error(String(error)));
