    ...
    (let ((burn-result (try! (as-contract (contract-call? lp-pool execute lp-amount (some OP_REMOVE_LIQUIDITY))))))
      ...
      (let ((token-b-result (try! (as-contract (contract-call? .multihop swap-1 
        ...
        (let ((token-a-amount (get dx burn-result))
              (token-b-amount (get amount-out token-b-result)))
  ...
  (let (
    ...
    (let ((burn-result (try! (as-contract (contract-call? lp-pool execute lp-amount (some OP_REMOVE_LIQUIDITY))))))
      ...
      (let ((token-a-result (try! (as-contract (contract-call? .multihop swap-1 
        ...
        ;; Step 4: Transfer tokens back to sender using provided token contracts
        (let ((token-a-amount (get amount-out token-a-result))
  ...
  (let (
    ...
    (let ((burn-result (try! (as-contract (contract-call? lp-pool execute lp-amount (some OP_REMOVE_LIQUIDITY))))))
      ...
      (let ((token-a-result (try! (as-contract (contract-call? .multihop swap-1 
        ...
        ;; Step 4: Transfer tokens back to sender
        (let ((token-a-contract (get token-a token-a-result))
              (token-b-contract (get token-b token-b-result))
              (token-a-amount (get amount-out token-a-result))
              (token-b-amount (get amount-out token-b-result)))
          
          (try! (as-contract (contract-call? token-a-contract transfer token-a-amount CONTRACT sender none)))
          (try! (as-contract (contract-call? token-b-contract transfer token-b-amount CONTRACT sender none)))
