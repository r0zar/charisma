...
import { callReadOnlyFunction, getContractInfo, getAccountBalances } from '@repo/polyglot';
...
async function getSubnetTokenContractId(sublinkContractId: string): Promise<string> {
    try {
        ...
        const sublinkMetadata = await fetchTokenFromCache(sublinkContractId);
 ...
 */
export async function getSubnetTokenBalance(
    ...
    try {
        ...
        if (tokenContractId === '.stx') {
            ...
            try {
                const tokenMetadata = await fetchTokenFromCache(tokenContractId);
                if (tokenMetadata) {
        ...
        return {
            success: false,
            error: "Could not find token balance for subnet contract in any source"
        };
    } catch (error) {
