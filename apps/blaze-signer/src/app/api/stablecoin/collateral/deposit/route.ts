...
import { addBalance, getBalance, total_supply, totalStableSupply, getTokenFees } from "@/lib/stablecoin/state";
...
export async function POST(req: Request) {
    try {
        ...
        return NextResponse.json({ deposited: amountTokens, newBalance: bal, collateralRatio: collateralUsdCents / (stableCents || 1) });
    } catch (err) {
        console.error(err);
