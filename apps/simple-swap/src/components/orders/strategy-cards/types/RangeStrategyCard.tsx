...
function unpricedLabel(legs: number, pairs: number): string {
    const parts = [legs ? `${legs} unpriced legs` : '', pairs ? `${pairs} unpriced pairs` : ''].filter(Boolean);
...
export const RangeStrategyCard: React.FC<RangeStrategyCardProps> = (props) => {
    ...
    useEffect(() => {
        ...
        priceSeries.fetchBulkPriceSeries([pairB], '30d')
            .then((bulk) => { if (!cancelled) setSeriesB(bulk[pairB]); })
    ...
    return (
        <BaseStrategyCard {...props} onClick={() => onToggleExpansion(id)}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                <div><div className="text-white/50 uppercase tracking-wider">Realized {seriesError ? '(prices unavailable)' : unpricedLabel(m.unpricedLegs, m.unpricedPairs)}</div><div className={`font-mono text-base ${m.realizedUsd >= 0 ? 'text-green-400' : 'text-orange-400'}`}>{seriesB ? usd(m.realizedUsd) : '—'}</div></div>
