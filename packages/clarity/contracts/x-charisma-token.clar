...
(define-read-only (get-name)         (contract-call? .charisma-token get-name))
(define-read-only (get-symbol)       (contract-call? .charisma-token get-symbol))
(define-read-only (get-decimals)     (contract-call? .charisma-token get-decimals))
(define-read-only (get-token-uri)    (contract-call? .charisma-token get-token-uri))
(define-read-only (get-total-supply) (contract-call? .charisma-token get-total-supply))
...
(define-public (deposit (amount uint) (recipient (optional principal)))
  (let ((sender tx-sender)
    ...
    (try! (contract-call? .charisma-token transfer amount sender (as-contract tx-sender) none))
...
(define-public (withdraw (amount uint) (recipient (optional principal)))
  (let ((owner tx-sender)
    ...
    (try! (as-contract (contract-call? .charisma-token transfer amount tx-sender recv none)))
