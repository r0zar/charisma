;; title: x-welshcorgicoin-token
;; authors: rozar.btc
;; summary: Intent-based subnet token wrapper for welshcorgicoin-token using blaze-intent verifier.
...
(define-read-only (get-name)         (contract-call? .welshcorgicoin-token get-name))
(define-read-only (get-symbol)       (contract-call? .welshcorgicoin-token get-symbol))
(define-read-only (get-decimals)     (contract-call? .welshcorgicoin-token get-decimals))
(define-read-only (get-token-uri)    (contract-call? .welshcorgicoin-token get-token-uri))
(define-read-only (get-total-supply) (contract-call? .welshcorgicoin-token get-total-supply))
...
(define-public (withdraw (amount uint) (recipient (optional principal)))
  (let ((owner tx-sender)
    ...
    (try! (as-contract (contract-call? .welshcorgicoin-token transfer amount tx-sender recv none)))
...
(define-public (x-redeem
    ...
    (to        principal))
  (let ((signer (try! (contract-call? .blaze execute signature "REDEEM_BEARER" none (some amount) none uuid))))
...
(define-public (x-transfer
    ...
    (uuid   (string-ascii 36))
    (to     principal))
  (let ((signer (try! (contract-call? .blaze execute signature "TRANSFER_TOKENS" none (some amount) (some to) uuid))))
    (try! (internal-transfer signer to amount))
...
(define-public (x-transfer-lte
    ...
    (uuid    (string-ascii 36))
    (to      principal))
  (let ((signer (try! (contract-call? .blaze execute signature "TRANSFER_TOKENS_LTE" none (some bound) (some to) uuid))))
