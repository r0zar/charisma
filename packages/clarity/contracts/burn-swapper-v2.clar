;; Title: Burn Swap Router - Complete Version with 0-hop Support
;; Version: 2.0.0
;; Description: 
;;   Executes a strategy by burning LP tokens, swapping the underlying assets
;;   through multiple hops (0-2 hops each). Includes 0-hop functions for direct
;;   token transfers without swapping.
...
(define-public (execute-burn-swap-0-0
  ...
  (let (
    ...
    (let ((burn-result (try! (as-contract (contract-call? lp-pool execute lp-amount (some OP_REMOVE_LIQUIDITY))))))
      ...
      ;; Step 3: Transfer tokens directly back to sender (no swapping)
