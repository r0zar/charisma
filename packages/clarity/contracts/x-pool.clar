...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://metadata.charisma.rocks/api/v1/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.sub-link"))
...
(define-public (swap-a-to-b
    ...
    (let ((delta (get-swap-quote amount OP_SWAP_A_TO_B)))
        ...
        (try! (as-contract (contract-call? .x-welshcorgicoin-token transfer (get dy delta) CONTRACT
            recipient none
        )))
        (ok delta)
    )
...
(define-public (swap-b-to-a
    ...
    (let ((delta (get-swap-quote amount OP_SWAP_B_TO_A)))
        ...
        (try! (contract-call? .x-welshcorgicoin-token transfer amount tx-sender
            CONTRACT none
        ))
        ;; Transfer token A to sender
        (try! (as-contract (contract-call? .x-charisma-token transfer (get dy delta) CONTRACT
            recipient none
        )))
        (ok delta)
    )
...
(define-public (add-liquidity
    ...
    (let ((delta (get-liquidity-quote amount)))
        (try! (contract-call? .x-charisma-token transfer (get dx delta) tx-sender
            CONTRACT none
        ))
        (try! (contract-call? .x-welshcorgicoin-token transfer (get dy delta) tx-sender
            CONTRACT none
        ))
...
(define-public (remove-liquidity
    ...
    (let ((delta (get-liquidity-quote amount)))
        ...
        (try! (as-contract (contract-call? .x-charisma-token transfer (get dx delta) CONTRACT
            recipient-a none
        )))
        (try! (as-contract (contract-call? .x-welshcorgicoin-token transfer (get dy delta) CONTRACT
            recipient-b none
...
(define-public (x-add-liquidity
        (amount uint)
        (signature-a (buff 65))
        (uuid-a (string-ascii 36))
        (recipient-a principal)
        (signature-b (buff 65))
        (uuid-b (string-ascii 36))
        (recipient-b principal)
    )
    (let (
        ...
        (try! (contract-call? .x-charisma-token x-transfer signature-a amount-a uuid-a
            CONTRACT
        ))
        (try! (contract-call? .x-welshcorgicoin-token x-transfer signature-b amount-b
            uuid-b CONTRACT
        ))
        (try! (ft-mint? sublink (/ (get dk delta) u2) recipient-a))
        (try! (ft-mint? sublink (/ (get dk delta) u2) recipient-b))
...
(define-public (x-remove-liquidity
    ...
    (let (
            (signer (try! (contract-call? .blaze execute signature "REMOVE_LIQUIDITY" none
                (some amount) none uuid
            )))
            (delta (get-liquidity-quote amount))
            (amount-a (get dx delta))
            (amount-b (get dy delta))
        ...
        (try! (as-contract (contract-call? .x-charisma-token transfer amount-a CONTRACT recipient
            none
        )))
        (try! (as-contract (contract-call? .x-welshcorgicoin-token transfer amount-b CONTRACT
            recipient none
        )))
        (ok delta)
    )
...
(define-public (x-swap-a-to-b
    ...
    (let ((delta (get-swap-quote amount OP_SWAP_A_TO_B)))
        ...
        (try! (contract-call? .x-charisma-token x-transfer signature amount uuid
            CONTRACT
        ))
...
(define-public (x-swap-b-to-a
    ...
    (let ((delta (get-swap-quote amount OP_SWAP_B_TO_A)))
        ;; Transfer tokens to contract from signer
        (try! (contract-call? .x-welshcorgicoin-token x-transfer signature amount uuid
            CONTRACT
        ))
        ;; Transfer tokens to recipient from contract
        (try! (as-contract (contract-call? .x-charisma-token transfer (get dy delta) CONTRACT
