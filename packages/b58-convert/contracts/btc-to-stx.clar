...
;;         (hash160 (unwrap-panic (as-max-len? (unwrap-panic (slice? decoded-bytes u1 u21)) u20)))
...
;;         ;; Create STX versioned data for checksum verification
;;         (versioned-data (concat btc-version hash160))
...
;;         (ok (unwrap-panic (principal-construct? stx-version hash160 none)))
;;     )
;; )
...
;;     (let (
;;         ;; Get the character list from string
;;         (char-list (string-to-chars addr))
        
;;         ;; Convert each character to its corresponding Base58 value
;;         (value-list (map char-to-uint char-list))
        
;;         ;; Decode the value list to bytes
;;         (bytes (base58-to-bytes value-list))
;;     )
;;         bytes
;;     )
;; )

;; ;; Convert Base58 character to its uint value
;; (define-read-only (char-to-uint (c (string-ascii 1)))
;;     (unwrap-panic (index-of? BASE58_CHARS c))
;; )

;; ;; Decode Base58 values to bytes
;; (define-read-only (base58-to-bytes (values (list 44 uint)))
;;     (let (
;;         ;; Count leading '1's (these represent leading zeros in the result)
;;         (leading-ones (count-leading-ones values u0))
        
;;         ;; Initial buffer with leading zeros
;;         (leading-zeros (generate-leading-zeros leading-ones))
        
;;         ;; Decode the non-zero part
;;         (decoded (fold decode-base58-digit (default-to LST (slice? values leading-ones (len values))) 0x00))
;;     )
;;         ;; Concatenate leading zeros with decoded bytes
;;         (concat leading-zeros decoded)
;;     )
;; )

;; ;; Count leading '1's in Base58 address
;; (define-read-only (count-leading-ones (input (string-ascii 50)))
;; (let (
;; (c1 (if (> (len input) u0)
;; (default-to "" (as-max-len? (default-to "" (slice? input u0 u1)) u1))
;; ""))
;; (c2 (if (> (len input) u1)
;; (default-to "" (as-max-len? (default-to "" (slice? input u1 u2)) u1))
;; ""))
;; (c3 (if (> (len input) u2)
;; (default-to "" (as-max-len? (default-to "" (slice? input u2 u3)) u1))
;; ""))
;; (c4 (if (> (len input) u3)
;; (default-to "" (as-max-len? (default-to "" (slice? input u3 u4)) u1))
;; ""))
;; (c5 (if (> (len input) u4)
;; (default-to "" (as-max-len? (default-to "" (slice? input u4 u5)) u1))
;; ""))
;; )
;; (+ (if (is-eq c1 "1") u1 u0)
;; (if (and (is-eq c1 "1") (is-eq c2 "1")) u1 u0)
;; (if (and (is-eq c1 "1") (is-eq c2 "1") (is-eq c3 "1")) u1 u0)
;; (if (and (is-eq c1 "1") (is-eq c2 "1") (is-eq c3 "1") (is-eq c4 "1")) u1 u0)
;; (if (and (is-eq c1 "1") (is-eq c2 "1") (is-eq c3 "1") (is-eq c4 "1") (is-eq c5 "1")) u1 u0))
;; ))

;; (define-read-only (generate-leading-zeros (count uint))
;; (if (is-eq count u1)
;; 0x00
;; (if (is-eq count u2)
;; 0x0000
...
;; (if (is-eq count u5)
;; 0x0000000000
;; 0x00)))))) ;; Default to buffer with no zeros

;; ;; Decode a Base58 digit
;; (define-read-only (decode-base58-digit (digit uint) (acc (buff 25)))
;;     (let (
;;         (current-len (len acc))
;;         (result (fold decode-digit acc (list digit)))
;;     )
;;         result
;;     )
;; )

;; ;; Inner function for decoding a digit
;; (define-read-only (decode-digit (digit uint) (acc (buff 25)))
;;     (let (
;;         (acc-len (len acc))
;;         (result (unwrap-panic (as-max-len? (if (<= acc-len u0)
;;                 (concat acc (if (> digit u0) (uint-to-buff digit) 0x))
;;                 (let (
;;                     (acc-with-carry (handle-decode-carry (concat acc 0x00) digit u0))
;;                 )
;;                     acc-with-carry
;;                 )
;;             ) u25)))
...
;; ;; Replace a byte at a specific position
;; (define-read-only (replace-byte-at-position (buff-input (buff 26)) (pos uint) (new-byte (buff 1)))
;;     (let (
;;         (before (default-to 0x (slice? buff-input u0 pos)))
;;         (after (default-to 0x (slice? buff-input (+ pos u1) (len buff-input))))
;;     )
;;         (unwrap-panic (as-max-len? (concat (concat before new-byte) after) u26))
;;     )
;; )
...
;;     (unwrap-panic (element-at? BASE58_CHARS index))
;; )
...
;; ;; ====================
;; ;; Begin Block to Test Helper Functions
;; ;; ====================
